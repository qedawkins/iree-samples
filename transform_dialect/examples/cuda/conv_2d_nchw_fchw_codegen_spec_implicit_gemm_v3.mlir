...
^bb1(%variant_op: !pdl.operation):
  ...
  %forall, %tiled_matmul =
    transform.iree.tile_to_forall_and_workgroup_count_region %matmul tile_sizes [1, 128, 128, 0]
  ...
  // Step 3. Tile + Fuse
  // ==============================================================
  %matmul_loopk, %loop = transform.structured.tile_to_scf_for %tiled_matmul [0, 0, 0, 128]
  ...
  // ==============================================================
  %forall_l3, %matmul_l3 =
  transform.structured.tile_to_forall_op %promoted_matmul_l2 num_threads [0, 8, 8]
    ( mapping = [#gpu.thread<y>, #gpu.thread<x>] )
  transform.structured.tile_to_forall_op %im2col_loopk num_threads [0, 8, 8]
    ( mapping = [#gpu.thread<y>, #gpu.thread<x>] )
  ...
  %func_11 = transform.iree.vector.vector_to_mma_conversion %func_10 { use_wmma }
  transform.iree.apply_patterns %variant_op_3 {canonicalization, cse, licm, tiling_canonicalization}
  transform.iree.erase_hal_descriptor_type_from_memref %func_11
}
